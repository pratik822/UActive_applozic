...
public class CreatePickUp extends Activity implements View.OnClickListener, OnTimeSetListener, ResponseListener,
    ...
    private void updatePickUp() throws IOException {
        if (Utility.isConnectingToInternet(CreatePickUp.this)) {
            ...
            try {
                params.put("area",addresses.get(0).getSubLocality());
        ...
        if (isEdit && isBooked.equalsIgnoreCase("1")) {
            ...
            builder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int id) {
                    AppConstants.showLocatoinPicker(CreatePickUp.this, true, activityList.getActivity().toString().trim(), activityList.isBookingOpen());
                    dialog.dismiss();
                }
            });
    ...
    public void getChannel(Channel mChannel) {
        ...
        if (Utility.isConnectingToInternet(CreatePickUp.this)) {
            try {
                ...
                addresses = geocoder.getFromLocation(Lat, lng, 1);
                Log.d("myadd",new Gson().toJson(addresses));

                try {
                    if(addresses.get(0).getSubLocality()!=null){
                        param.put("area",addresses.get(0).getSubLocality());
                    }else{
                        param.put("area","");
                    }
