    ...
    public void successResponse(String successResponse, int flag) throws JSONException {
        ...
        switch (flag) {
                    ...
                    if (jsonObject.optString(resultcheck).equals(KEY_TRUE)) {
                        ...
                        if (getfav != null) {
                            for (int i = 0; i < getfav.length(); i++) {
                                ...
                                arrayList.add(new FavouriteModel(favObj.optString("iduser"), favObj.optString("firstname") + " " + favObj.optString("lastname"), favObj.optString("image")));
                            }
                            listView.invalidate();
                            if (isEdit) {
            ...
            case 3:
                if (jsonObject != null) {
                    if (jsonObject.optString(resultcheck).equals(KEY_TRUE)) {
                        ...
                        JSONObject page = new JSONObject(jsonObject.getJSONObject("details").getJSONObject("near_by").getString("pageDetail"));
            ...
            case 5:
                ...
                if (jsonObject != null) {
                    if (jsonObject.optString(resultcheck).equals(KEY_TRUE)) {
                        JSONObject details = new JSONObject(jsonObject.getJSONObject("details").getString("near_by_search"));
                        // mInviteListsModel = Utility.getModelFromJson(jsonObject.toString(), InviteListsModel.class);
                        //   mInviteListsModel = Utility.getModelFromJson(jsonObject.toString(), InviteListsModel.class);
...
//                setAdapter();
    ...
    public void invite_search(String searchText) {
        ...
        RequestHandler.getInstance().stringRequestVolley(InvitePeopleFav.this, AppConstants.getBaseUrl(SharedPref.getInstance().getBooleanValue(InvitePeopleFav.this, isStaging)) + "get_nearby_search_buddy", param, this, 5);
    }
